...
def detect_and_draw_contours(main_frame, mask, color, divider ,min_area=50, min_radius=10, max_radius=200):
    count_circle = 0
    ...
    for contour in contours:
        ...
        if area > min_area:  # Filter for circular shapes with a minimum area
            ...
            center = (int(x), int(y))
            radius = int(radius)
            if int(min_radius) <= int(radius) <= int(max_radius):  # Check if radius is within the specified limits
                count_circle += 1
                # print(f'circle = {count_circle}')
                if(count_circle < 2):
                    cv2.circle(main_frame, center, radius, color, 1)
                    cv2.circle(main_frame, center, int(radius/divider), color, 1)
    ...
    while(video.isOpened()):
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                ...
                if circle_center_9 and circle_center_10 : 
                    cv2.circle(image, circle_center_9, radius_9, (255, 0, 255), 1)
                    cv2.circle(image, circle_center_10, radius_10, (255, 0, 255), 1)
                    ...
                    lines = cv2.HoughLinesP(arrow, 1, np.pi / 180, threshold=150, minLineLength=10, maxLineGap=30)
                    count_line = 0    
                        ...
                        for line in lines:
                            ...
                            if((y1 > upperBound and y1 < lowerBound and x1 > leftBound and x1 < rightBound) or 
                               (y2 > upperBound and y2 < lowerBound and x2 > leftBound and x2 < rightBound)):
                                ...
                                is_inside_1 = int(tip_pos_1) <= int(radius_1) or int(tip_pos_1_end) <= int(radius_1)
                                is_inside_2 = int(tip_pos_2) <= int(radius_2) or int(tip_pos_2_end) <= int(radius_2)
                                is_inside_3 = int(tip_pos_3) <= int(radius_3) or int(tip_pos_3_end) <= int(radius_3)
                                is_inside_4 = int(tip_pos_4) <= int(radius_4) or int(tip_pos_4_end) <= int(radius_4)
                                is_inside_5 = int(tip_pos_5) <= int(radius_5) or int(tip_pos_5_end) <= int(radius_5)
                                is_inside_6 = int(tip_pos_6) <= int(radius_6) or int(tip_pos_6_end) <= int(radius_6)
                                is_inside_7 = int(tip_pos_7) <= int(radius_7) or int(tip_pos_7_end) <= int(radius_7)
                                is_inside_8 = int(tip_pos_8) <= int(radius_8) or int(tip_pos_8_end) <= int(radius_8)
                                is_inside_9 = int(tip_pos_9) <= int(radius_9) or int(tip_pos_9_end) <= int(radius_9)
                                is_inside_10 = int(tip_pos_10) <= int(radius_10) or int(tip_pos_10_end) <= int(radius_10)
